...
async def get_channel_members(session, token, channel_id):
    ...
    try:
        async with session.get(url, headers=headers) as response:
            if response.status == 200:
                ...
                return list(set(msg["author"]["id"] for msg in messages if not msg["author"].get("bot")))
    except Exception:
        pass
    return []

async def send_message_worker(session, token, channel_id, base_text, use_rand_str, rand_str_len, use_mention, member_ids, delay, count):
        ...
        try:
            async with session.post(url, headers=headers, json=payload) as res:
                if res.status in [200, 201]:
                    log_success(f"送信成功 {masked_token}")
                elif res.status == 429:
                    retry_after = (await res.json()).get("retry_after", 1)
                    log_error(f"速度制限中: {retry_after}秒待機 {masked_token}")
                    await asyncio.sleep(retry_after)
                else:
                    log_error(f"送信失敗 {masked_token} (Status: {res.status})")
        except Exception as e:
            log_error(f"送信失敗 {masked_token} (エラー: {e})")
        if delay > 0:
            await asyncio.sleep(delay)

async def messenger_menu(session, tokens):
    from utils import log_info, log_success, log_error
    print(f"\n{'='*50}")
    print("Discord Messenger")
    print(f"{'='*50}")
    ...
    log_success("送信を開始します...")
    print()
    tasks = []
    for token in tokens:
        tasks.append(send_message_worker(session, token, channel_id, base_text, use_rand_str, rand_str_len, use_mention, member_ids, send_delay, send_count))
