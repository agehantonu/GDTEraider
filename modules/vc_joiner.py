...
async def vc_join_worker(session, token, guild_id, channel_id, ja3_profile=None):
    ...
    try:
        async with session.patch(url, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=15)) as res:
            if res.status in [200, 201, 204]:
                log_success(f"VC参加成功 {masked_token} → Channel: {channel_id}")
                return True
            elif res.status == 429:
                ...
                log_error(f"VC参加禁止（権限不足） {masked_token}")
                return "forbidden"
            elif res.status == 404:
                log_error(f"VCチャンネル不存在 {masked_token}")
                return "not_found"
            else:
                log_error(f"VC参加失敗 {masked_token} (Status: {res.status})")
                return False
    except Exception as e:
        log_error(f"VCエラー {masked_token}: {e}")
        return False

async def vc_joiner_menu(session, tokens):
    print(f"\n{'='*50}")
    print("Discord VC Joiner")
    print(f"{'='*50}")
    guild_id = input("サーバーIDを入力: ").strip()
    channel_id = input("ボイスチャンネルIDを入力: ").strip()
    ...
    log_success(f"VC参加を開始します... (Guild: {guild_id} | Channel: {channel_id})")
    print()
        ...
        if ja3_profile or proxy_url:
            ...
            async with aiohttp.ClientSession(connector=connector) as ja3_session:
                for attempt in range(max_retries):
                    result = await vc_join_worker(ja3_session, token, guild_id, channel_id, ja3_profile)
                    if result == True:
            ...
            for attempt in range(max_retries):
                result = await vc_join_worker(session, token, guild_id, channel_id, None)
                if result == True:
                    success_count += 1; break
                elif result == "invalid_token":
                    invalid_tokens.append(token); fail_count += 1; break
